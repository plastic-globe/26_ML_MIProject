...
### Build The Dataset Tree

The repository does not ship the processed `lib/` data files. Generate them from a local MMLU-style file or from Hugging Face before running experiments.

```bash
# Option 1: build from a local file
python experiments/data_generation/build_lib.py \
  --input-file raw_data/sample_mmlu.jsonl

# Option 2: build from Hugging Face
python experiments/data_generation/build_lib.py \
...
- `lib/pov/...` for first-person and third-person variants
...
### 2.5 Steering Analysis

This repository now includes a minimal activation-steering pipeline that learns a direction from aligned `plain` and `opinion_only` prompts, then injects that direction at a target decoder layer during inference.
...
Outputs are written under `output_steering/mmlu/activation_steering/` and include:

- a `.pkl` file with baseline vs. steered answers
- a `.json` file with accuracy and sycophancy deltas

You can summarize completed steering runs with:
...
The repository now includes mitigation-oriented `improve` workflows that compare baseline behavior against improved prompting and tuned steering settings.
...
Set `PYTHON_EXECUTABLE` if you want the sweep to use a specific Python runtime.
